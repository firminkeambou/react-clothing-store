...
import { persistStore, persistReducer, PersistConfig } from 'redux-persist'; // to persist our store on the browser storage
...
declare global {
  interface Window {
    __REDUX_DEVTOOLS_EXTENSION_COMPOSE__?: typeof compose; // optional as the browser may not have the devtool
  }
}

type ExtendedPersistConfig = PersistConfig<RootState> & {
  whitelist: (keyof RootState)[];
}; // this definition help us to fix the only values acceptable by whitelist

//config object for redux-persist, only needed if we want to persist our store
const persistConfig: ExtendedPersistConfig = {
  key: 'root', // key for the local storage, the top level of our state object, so the persisted object will have as name "root" in the local storage in the browser
...
/* look at the typescript version of the middleware just right after this commented block
const middlewares = [
...
//typscript version of the middleware

const middlewares = [
  process.env.NODE_ENV !== 'production' && logger,
  sagaMiddleware,
].filter((middleware): middleware is Middleware => Boolean(middleware)); // creates an object like {true}, {false} ." middleware is Middleware" reminds us predicate

...
import { Category, CategoryMap } from './category.action-types';
import { CategorieState } from './category.reducer';
import { createSelector } from 'reselect';

const selectCategoryReducer = (state: RootState): CategorieState =>
  state.categories; // returning category state;the result s a categoriesSlice
...
export const selectCategories = createSelector(
  ...
  (categoriesSlice: CategorieState): Category[] => categoriesSlice.categories // output selector ; categoriesSlice is the result gets from selectCategoryReducer: we can have as many parameters as the number of input selectors
);

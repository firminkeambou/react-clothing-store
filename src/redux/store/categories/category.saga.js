...
export function* fetchCategoriesAsync() {
  try {
    const categoriesArray = yield call(getCategoriesAndDocuments); // yield means  wait(await) until something come back ; or call(getCategoriesAndDocuments,'categories')  if we see/consder "categories" as a parameter
    yield put(fetchCategoriesSuccess(categoriesArray)); // put replaces dispatch, so an action is dispatched here
  } catch (error) {
    yield put(fetchCategoriesFailed(error)); // another action is dispatched here
  }
}

...
import { Category } from '../../redux/store/categories/category.action-types';
import {
...
export const addCollectionAndDocuments = async <T extends ObjectToAdd>( // here , we ensure that ObjectToAdd has at least the field string
...
export const createUserDocumentFromAuth = async (
  ...
  //were are saying that the cliche contains "UserData"
  ...
  if (!userSnapshot.exists()) {
    ...
    try {
      ...
      console.log('error creating the user', error); // logs any errors that occur while setting the document
    }
  }
  return userSnapshot as QueryDocumentSnapshot<UserData>; // returns the user document reference, which can be used to access the user's document in the Firestore database
};

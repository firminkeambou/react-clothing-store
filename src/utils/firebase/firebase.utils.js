...
const provider = new GoogleAuthProvider(); // creates a new instance of the GoogleAuthProvider class, which is used to authenticate users with their Google accounts
...
export const signInWithGooglePopup = () => signInWithPopup(auth, provider); // exports a function that allows users to sign in with a Google popup window
export const signInWithGoogleRedirect = () => signInWithRedirect(auth, provider); // exports a function that allows users to sign in with a Google redirect, which means the user will be redirected to the Google sign-in page and then back to your app after authentication
...
export const db = getFirestore(); // initializes the Firestore database instance using the Firebase
...
export const createUserDocumentFromAuth = async (userAuth) => {
  ...
  if (!userSnapshot.exists()) { // if the document does not exist, it means the user is signing up for the first time
    ...
    try {
      await setDoc(userDocRef, { // sets the document in the Firestore database with the user's information
        displayName,
        email,
        createdAt,
      });

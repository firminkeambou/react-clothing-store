import React, { ChangeEvent, FormEvent } from 'react'; //, { useContext }
import { AuthError, AuthErrorCodes } from 'firebase/auth';
//import { useDispatch } from 'react-redux';
import { useState } from 'react';
import { useSelector } from 'react-redux';
import FormInput from '../form-input/form-input.component';
import { createUserEmailPasswordStart } from '../../redux/store/user/user.action';
import {
  selectErrorSignUP,
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if ((error as AuthError).code === AuthErrorCodes.EMAIL_EXISTS) {
        // we are know using error as AuthError from firebase , no more directly "'auth/email-already-in-use'"
    ...
    <SignUpContainer>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ paddingBottom: 24 }}>
          {errorSignUp &&
            errorSignUp.code &&
            errorSignUp.code === AuthErrorCodes.EMAIL_EXISTS && ( //'auth/email-already-in-use'
